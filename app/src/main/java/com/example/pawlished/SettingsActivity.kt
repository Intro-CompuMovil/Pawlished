...
class SettingsActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        elegirImagenButton.setOnClickListener {
            ...
            if (checkGalleryPermission()) {
                startActivityForResult(intent, SettingsActivity.IMAGE_PICK_REQUEST)
            } else {
    ...
    private fun requestCameraPermission() {
        ActivityCompat.requestPermissions(
            ...
            SettingsActivity.CAMERA_PERMISSION_REQUEST
        )
    }
    private fun launchCamera() {
        val intent = Intent(MediaStore.ACTION_IMAGE_CAPTURE)
        startActivityForResult(intent, SettingsActivity.IMAGE_CAPTURE_REQUEST)
    }
    override fun onRequestPermissionsResult(
        ...
        if (requestCode == SettingsActivity.CAMERA_PERMISSION_REQUEST) {
    ...
    override fun onActivityResult(requestCode: Int, resultCode: Int, data: Intent?) {
        ...
        if (requestCode == SettingsActivity.IMAGE_CAPTURE_REQUEST && resultCode == RESULT_OK)
        {
            val imageBitmap = data?.extras?.get("data") as Bitmap
            val rotatedBitmap = rotateImage(imageBitmap, 90f)
            foto.setImageBitmap(rotatedBitmap)
        }
        else if (requestCode == SettingsActivity.IMAGE_PICK_REQUEST && resultCode == RESULT_OK)
        {
    ...
    private fun requestGalleryPermission() {
        ActivityCompat.requestPermissions(
            ...
            SettingsActivity.GALLERY_PERMISSION_REQUEST
        )
    }
